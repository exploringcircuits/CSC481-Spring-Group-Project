...
class PlayerViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def filter_queryset(self, queryset):
        ...
        # Always apply text search first so name filtering works regardless of sort.
    ...
    def teams(self, request):
        ...
        raw_teams = Player.objects.values_list("team_abbreviation", "team_name")

        # Deduplicate by team abbreviation so each NBA team appears exactly once.
        # Keep the first non-empty name found for the abbreviation.
        unique_teams = {}
        for abbreviation, name in raw_teams:
            if not abbreviation:
                continue

            if abbreviation not in unique_teams:
                unique_teams[abbreviation] = name or abbreviation

        sorted_teams = sorted(unique_teams.items(), key=lambda item: item[0])

        return Response(
            [
                {"abbreviation": abbreviation, "name": team_name}
                for abbreviation, team_name in sorted_teams
            ]
        )
        
class RegisterView(APIView):
    def post(self, request):
        # Pass the frontend JSON data (request.data) to the Serializer
        serializer = RegisterSerializer(data=request.data)
        
        if serializer.is_valid():
            # This calls the create() method in your serializer and saves to db.sqlite3
            serializer.save() 
            return Response({"message": "User registered successfully!"}, status=status.HTTP_201_CREATED)
        
        # If passwords don't match or username is taken, send the errors back to TypeScript
